...
export default function HistorialDetailPage() {
  ...
  if (loading) return (
    <div className="flex flex-col justify-center items-center min-h-screen gap-4">
      ...
      <p className="text-[14px] text-[#4d453e]">Cargando análisis...</p>
    </div>
  )
  ...
  if (!analysis) return (
    <div className="flex flex-col justify-center items-center min-h-screen gap-4">
      ...
      <p className="text-[15px] text-[#1b1c1c]">Análisis no encontrado</p>
      <button onClick={() => router.push('/historial')} className="text-[14px] text-[#9BA88F]">← Volver al historial</button>
    </div>
  )
  ...
  const vConfig = {
    ...
    NO_PUBLICAR: { bg: '#FF3B30', icon: I.flor(20),   label: 'NO PUBLICAR' }
  }[analysis.veredicto] || { bg: '#34C759', icon: I.piedra(20), label: 'APROBADO' }

  const lines = analysis.explicacion ? analysis.explicacion.split('\n').filter(l => l.trim()) : []
  ...
  const getLineIcon = (line: string) => {
    ...
    if (l.startsWith('composicion:') || l.startsWith('composición:')) return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c5.5 0 10-4.5 10-10S17.5 2 12 2 2 6.5 2 12c0 3.5 2 6 5 6s5-2.5 5-5-1.5-4-4-4"/></svg>
    if (l.startsWith('publico:') || l.startsWith('público:')) return I.ojo(13)
    if (l.startsWith('funciona:')) return I.piedra(13)
    if (l.startsWith('mejorar:')) return I.hoja(13)
    return I.luna(13)
  }
    ...
    <body className="text-on-surface antialiased min-h-screen flex flex-col" style={{ background: 'linear-gradient(135deg, rgba(143, 168, 149, 0.12) 0%, #FFFFFF 50%, rgba(143, 168, 149, 0.1) 100%)' }}>
      ...
      <header className="fixed top-0 left-0 w-full z-50 flex justify-center bg-[rgba(251,249,248,0.9)] backdrop-blur-md border-b border-[#d0c4bb]">
        <div className="w-full max-w-[900px] flex justify-between items-center px-[20px] py-3">
          ...
          <a href="/historial" className="text-[14px] font-medium text-[#4d453e] hover:text-[#695c4f] flex items-center gap-1.5">
            <span className="text-[#9BA88F]">{I.hoja(13)}</span> Mis análisis
          </a>
        </div>
      </header>
      ...
      <main className="flex-grow flex items-start justify-center pt-20 pb-8 px-[20px] w-full relative z-10">
        <div className="w-full max-w-[700px] flex flex-col gap-4 py-4">
          ...
          {/* Fecha */}
          <p className="text-[12px] text-[#9BA88F] text-center">{formatDate(analysis.created_at)}</p>
          ...
          <div className="flex gap-4">
            ...
            <div className="w-[90px] bg-white border border-[#E5E5EA] rounded-xl flex flex-col items-center justify-center gap-1">
              <div className="text-[#9BA88F] opacity-40">{I.piedra(13)}</div>
          ...
          <div className="flex gap-4">
            <div className="flex-1 bg-white border-l-4 border-[#9BA88F] rounded-xl p-4">
              ...
              <p className="text-[13px] text-[#4d453e] italic leading-[20px] mb-3">
                {analysis.caption_sugerido || 'No disponible'}
              </p>
              {analysis.caption_sugerido && (
                <button onClick={() => copy(analysis.caption_sugerido, 'caption')} className="text-[12px] font-medium text-[#9BA88F] flex items-center gap-1.5">
            ...
            <div className="flex-1 bg-[#f6f3f2] border border-[#E5E5EA] rounded-xl p-4">
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {(hashtagsMap[analysis.veredicto] || hashtagsMap.APROBADO).split(' ').map((tag, idx) => (
                  <span key={idx} className="px-2 py-0.5 bg-white border border-[#E5E5EA] rounded-full text-[11px] text-[#695c4f] font-medium">{tag}</span>
                ))}
              </div>
              <button onClick={() => copy(hashtagsMap[analysis.veredicto] || hashtagsMap.APROBADO, 'tags')} className="text-[12px] font-medium text-[#9BA88F] flex items-center gap-1.5">
          ...
          <div className="flex gap-3 justify-center pt-1">
            ...
            <button onClick={() => router.push('/content-generator')} className="px-5 py-2.5 bg-[#9BA88F] text-white rounded-lg text-[14px] font-medium hover:brightness-105 flex items-center gap-2">
              <span>{I.hoja(14)}</span> Generar contenido
            </button>
          </div>
